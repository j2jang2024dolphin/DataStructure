...
public class ExpressionTree {
    ...
    public static BinaryTree construct(String exp) {
        ...
        if (Character.isDigit(exp.charAt(0))) {
            root.setLeft(exp.charAt(0));
        } 
       else {
            int index = closingIndex(exp, 0);
            BinaryTree left = construct(exp.substring(0, closingIndex(exp, 0));
            left.parent = root;
            root.left = left;
        ...
        if (Character.isDigit(exp.charAt(exp.length() - 1))) {
            ...
            BinaryTree right = construct(exp.substring(exp.charAt(opIndex + 1), exp.length() - 1));
            right.parent = root;
            root.right = right;
        }
